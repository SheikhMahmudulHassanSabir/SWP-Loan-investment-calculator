...
function calculate() {
  ...
  for (let m = 1; m <= months; m++) {
    ...
    let available        = corpus + interestEarned;
    
    if (available >= emi) {
      corpus = available - emi;
    } else {
      outOfPocket += (emi - available);
      corpus = 0;
    }
  ...
  let profitLoss = corpus - outOfPocket;
  if (Math.abs(profitLoss) < 0.01) profitLoss = 0;
  ...
  let plFormatted;
  if (profitLoss > 0) plFormatted = '+' + formatCurrency(profitLoss);
  else if (profitLoss < 0) plFormatted = '−' + formatCurrency(Math.abs(profitLoss));
  else plFormatted = formatCurrency(0);
  
  document.getElementById('profitLossResult').textContent = plFormatted;
